...
const {
  ...
  getTokenSymbol,
  getWellName,
} = require("./wells/wellDeploymentUtils");
const {
  ...
  askExitWithInitData,
  padPumpData,
} = require("./wells/wellDeploymentInput");
...
async function deployWell() {
  ...
  console.log(
    "This script will guide you through the deployment of a new well on Basin DEX."
  );
  console.log(
    "Basin is a composable DEX. This means you can compose exchange functions, network-native oracles (pumps) and exchange implementations into a single liquidity pool known as a Well however you wish.\n"
  );
  console.log(
    "Note that Basin is under development and limited components are available at this time.\n"
  );
  console.log(
    "More information can be found at: https://docs.basin.exchange/ \n"
  );
  ...
  const {
    ...
    pump,
    wellImplementation,
  } = await inquirer.prompt(componentQuestions);

  // safeguards
  validateWellInput(token1Address, token2Address);

  // map the component details
  const wellFunctionDetails = mapComponentDetails(
    "wellFunction",
    wellFunction,
    network
  );
  ...
  const implementationDetails = mapComponentDetails(
    "wellImplementation",
    wellImplementation,
    network
  );
  ...
  const dataQuestions = await getWellDataQuestionsArray(
    token1Address,
    token2Address,
    wellFunctionName,
    wellFunctionSymbol
  ...
  let {
    ...
    maxLpSupplyIncrease,
    maxLpSupplyDecrease,
  } = await inquirer.prompt(pumpDataQuestions);
  [
    alpha,
    capInterval,
    maxRateChanges,
  ...
  const pumpData = await encodePumpData(
    alpha,
    capInterval,
  ...
  const deployedAquifierAddress =
    network === "Mainnet" ? aquifierAddressMainnet : aquifierAddressSepolia;

  // Aquifier abi from etherscan
  const aquifierABI = JSON.parse(
    await fs.readFileSync("contracts/Aquifier_ABI.json", "utf8")
  );

  // get deployed aquifier contract
  const deployedAquifier = await hre.ethers.getContractAt(
    aquifierABI,
    deployedAquifierAddress
  );
  ...
  const wellImplementationABI = JSON.parse(
    await fs.readFileSync("contracts/Well_ABI.json", "utf8")
  );
  ...
  console.log(
    `View the Aquifer transaction on Etherscan: https://${
      network === "Mainnet" ? "etherscan.io" : "sepolia.etherscan.io"
    }/address/${deployedAquifierAddress}`
  );
}
