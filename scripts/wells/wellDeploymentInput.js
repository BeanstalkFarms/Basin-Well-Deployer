...
const {
  ...
  getWellSymbol,
  getTokenSymbol,
} = require("./wellDeploymentUtils");
const inquirer = require("inquirer");
const fs = require("fs");

// Sepolia addresses
const aquifierAddressSepolia = "0x7aa056fCEf8F529E8C8e0732727F40748f49Bc1B";
const constantProduct2AddressSepolia =
  "0x30Af8bc21683754086CDcb27C828FACAE85Cbcad";
const multiFlowPumpAddressSepolia =
  "0x9A3AE9143753eFc2cC9583Eb03DEFaC75b649686";
const standardWellImplementationAddressSepolia =
  "0x29F2f6EBA43ecE36a355D786FbF7Aa34432dB780";

// Mainnet addresses
const aquifierAddressMainnet = "0xBA51AAAA95aeEFc1292515b36D86C51dC7877773";
const constantProduct2AddressMainnet =
  "0xBA150C2ae0f8450D4B832beeFa3338d4b5982d26";
const multiFlowPumpAddressMainnet =
  "0xBA51AaaAa95bA1d5efB3cB1A3f50a09165315A17";
const standardWellImplementationAddressMainnet =
...
function getWellComponentQuestionsArray() {
  return [
    ...
    },
    {
      ...
      message:
        "Select a well function to use for the Well. Read more about well functions at: https://docs.basin.exchange/components/well",
      name: "wellFunction",
    },
    {
      type: "list",
      ...
      message:
        "Select a pump to use for the Well (None for no pump). Read more about pumps at: https://docs.basin.exchange/components/pump",
      name: "pump",
    },
    {
      type: "list",
      ...
      message:
        "Select a well implementation to use for the Well. Read more about well implementations at: https://docs.basin.exchange/components/well#well-implementation",
      name: "wellImplementation",
    },
  ];
}

async function getWellDataQuestionsArray(
  token1Address,
  token2Address,
  wellFunctionName,
  wellFunctionSymbol
  ...
  return [
    {
      ...
      name: "wellName",
      message:
        "Input a name for the well, press ENTER to use the default name.",
      default: await getWellName(
        token1Address,
        token2Address,
        wellFunctionName
      ),
    },
    {
      ...
      name: "wellSymbol",
      message:
        "Input a symbol for the well, press ENTER to use the default symbol.",
      default: await getWellSymbol(
        token1Address,
        token2Address,
        wellFunctionSymbol
      ),
    },
    {
      ...
      name: "salt",
      message:
        "Input a salt for the well. Use a salt deploy a new Well with CREATE2 opcode. Press ENTER to add the default salt. Salts are used for vanity addresses and to predict the new well address.",
    },
  ];
}
...
async function padPumpData(
  ...
  alpha = await alpha;
  capInterval = await capInterval;
  for (let i = 0; i < maxRateChanges.length; i++) {
    for (let j = 0; j < maxRateChanges[i].length; j++) {
      maxRateChanges[i][j] = await maxRateChanges[i][j];
    }
  }
  maxLpSupplyIncrease = await maxLpSupplyIncrease;
  maxLpSupplyDecrease = await maxLpSupplyDecrease;
  return [
    alpha,
    capInterval,
    maxRateChanges,
  ...
  const pumpDataQuestions = [
    {
      ...
      default: "0x3ffeef368eb04325c526c2246eec3e55",
    },
    {
      type: "input",
      name: "capInterval",
    ...
    },
    {
      ...
      default: "0x3ff33a92a305532617c1bda5119ce075",
    },
    {
      type: "input",
...
async function printWellDefinition(
  token1Address,
  token2Address,
  wellFunctionAddress,
  pumpAddress,
  ...
  console.log(
    "Token1: ",
    token1Address + " (" + (await getTokenSymbol(token1Address)) + ")"
  );
  console.log(
    "Token2: ",
    token2Address + " (" + (await getTokenSymbol(token2Address)) + ")"
  );
  ...
  const componentConfigs = {
    wellFunction: {
      ConstantProduct2: {
        address: (network) =>
          network === "Mainnet"
            ? constantProduct2AddressMainnet
            : constantProduct2AddressSepolia,
    ...
    pump: {
      ...
      "Multi Flow Pump (recommended)": {
        address: (network) =>
          network === "Mainnet"
            ? multiFlowPumpAddressMainnet
            : multiFlowPumpAddressSepolia,
    ...
    wellImplementation: {
      "Standard Well Implementation (recommended)": {
        address: (network) =>
          network === "Mainnet"
            ? standardWellImplementationAddressMainnet
            : standardWellImplementationAddressSepolia,
...
function mapComponentDetails(componentType, key, network) {
  ...
  return {
    address:
      typeof detailConfig.address === "function"
        ? detailConfig.address(network)
