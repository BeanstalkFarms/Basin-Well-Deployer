...
contract MockSalt {
    ...
    uint256 constant prefixCharactersLength = 2;
    ...
    bytes constant targetPrefix=hex"ab";

    constructor(address _wellDeployer, uint256 _maxIterations) {
    ...
    function checkForAddress(uint256 start) external view returns (address a, string memory str, bytes32 s) {
        ...
        uint256 end = start + maxIterations;
    ...
    function checkSalt(bytes32 salt, bytes32 targetHash) internal view returns (address, string memory) {
        ...
        string memory wellString = toHexString(well, prefixCharactersLength);
